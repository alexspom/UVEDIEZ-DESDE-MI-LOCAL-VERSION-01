...
PREQUERY=FEJECUTA(FIF("CSELASIGNADO",FPOSICIONABLOQUE("VDPICKUNIDEC.PAN")),"",
                  "+CINITCAMPOS","")
...
CURSOR=CSELASIGNADO SELECT BUC.CODBULTO,BUC.CODPED,BUC.NBULTO
                     FROM VDBULTOLIN BUL,VDMOVIM MOV,VDBULTOCAB BUC
                     WHERE BUC.CODBULTO=BUL.CODBULTO AND BUL.CODMOV=MOV.CODMOV AND MOV.STATUS=VDST.FMOVASIGNADO AND MOV.CODRECURSO=:CODRECURSO;                     

CURSOR=CSELBULTO SELECT CODBULTO
                   FROM VDBULTOCAB 
                  WHERE CODBULTO=:CODBULTO OR CODBULTO = :CODBULTOIN;

CURSOR=CCHECKBULTO SELECT CODBULTO,CODPED,NBULTO
                     FROM VDBULTOCAB
