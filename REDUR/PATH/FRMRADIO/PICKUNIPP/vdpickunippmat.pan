...
CURSOR=CINITCAMPOS SELECT '' MATCAJA, '' CODBULTO,VD.GETPROP('LETIQMAT') LONMAT,0 ORDENSALIDA,'MONOPEDIDO' TEXTOTAREA, '' MATENCURSO FROM DUAL;

CURSOR=CMATENCUERSO SELECT 'MAT. EN CURSO: '||LISTAGG(MATCAJA,', ') WITHIN GROUP (ORDER BY MATCAJA) MATENCURSO
					  FROM VDBULTOCAB BUC, VDPEDIDOCAB PEC, VDBULTOLIN BUL, VDMOVIM MOV
					 WHERE PEC.CODDIV = BUC.CODDIV
...
CURSOR=CSELBULTO SELECT BUC.CODBULTO
                  ...
                  WHERE BUC.STATUS=VDST.FBUCRESERVADO AND BUL.CODBULTO=BUC.CODBULTO AND BUL.CODMOV=MOV.CODMOV AND 
                        UBI.CODUBI=MOV.CODUBIORI AND BUC.TIPOBULTO='U' AND BUC.TIPOVOL=VD.GETPROP('VOLDINAMICA') AND MOV.STATUS<=VDST.FMOVACTUALIZA AND 
                        ((MOV.STATUS=VDST.FMOVASIGNADO AND MOV.CODRECURSO = :CODRECURSO) OR MOV.STATUS=VDST.FMOVPDTERECOGE OR BUC.MATCAJA=:MATCAJA) AND 
                        ...
                        SEP.CODSERIEPREP=PEC.CODSERIEPREP AND NVL(PEC.SWTPREPMONO,'N')='S'
                   GROUP BY BUC.CODBULTO,BUC.MATCAJA,SEP.PRIORIDAD,PEC.PRIORIDAD,MOV.STATUS
                   ORDER BY (CASE WHEN :MATCAJA=BUC.MATCAJA THEN 0 ELSE 1 END),
...
CURSOR=CEXISTEBULTO SELECT :MATCAJA FROM VDBULTOCAB 
                     WHERE MATCAJA=:MATCAJA AND CODBULTO!=:CODBULTO AND STATUS<VDST.FBUCFINALIZADO;
...
CURSOR=CUPDMOVS UPDATE VDMOVIM
                   SET STATUS=VDST.FMOVASIGNADO,
                       ...
                       CODOPEMODIF=:CODOPE,
                       FECPLANMOV=VD.FECHASYS,
                       HORAPLANMOV=VD.HORASYS,
                 ...
                 WHERE CODMOV IN
                       (SELECT BUL.CODMOV FROM VDBULTOLIN BUL,VDMOVIM MOV 
                         WHERE BUL.CODBULTO=:CODBULTO AND BUL.CODMOV=MOV.CODMOV AND
                               MOV.STATUS+0=VDST.FMOVPDTERECOGE AND MOV.TAREA=VD.GETPROP('TAREAPEDUNIPP'));
