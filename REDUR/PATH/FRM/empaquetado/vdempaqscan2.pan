...
POSTINSERT=FEJECUTA("CVERCANTIDAD","Debe introducir cantidad o la cantidad introducida debe ser correcta",
                    "CMUEVE","No puedo mover",
                    "CRECALC","No puedo recalcular peso",
                    FIF("-CNOESFINEMPAQ",FEJECUTA("CTERMINABULTO","NO PUEDO TERMINAR EL BULTO",
                                                  "CBORRAORIG","NO PUEDO BORRAR BULTO ORIGINAL",
                                                  ...
                                                  FCOMMIT,"",
												  FIF("CULTIMOBUL",FEJECUTA(FIMPRIMEPEDIDO("INFORME",":CODDIV",":ANOPED",":CODPED",":SEQPED","$(VDALBARAN)","CODDIV=:CODDIV;ANOPED=:ANOPED;CODPED=:CODPED;SEQPED=:SEQPED",""),":V10ERROR",
                                                                            FCOMMIT,"",
                                                                            %FFAILURE,"PEDIDO TERMINADO\n INCLUIR ALBARAN\n :DATOSEXTRA \n :DATOSEXTRA2"),
																   %FFAILURE,"BULTO TERMINADO"),"",
                                                  FPULSATECLAS("ESC","F5"),"")
                                     ),"ERROR EN FIF")
...
CAMPO=CODART,TITULO("Artículo"),POSTCHANGE=FIF(-FESNUEVO,FDESIGNACION("@CESNULO","","CSELARTICULO","No existe artículo",
                                                                      "CSELCANTIDAD","Artículo no esta pendiente",
																	  FPULSATECLAS("TAB"),""))
...
CAMPO=PESOTEORICO,NOUPDATE,VIRTUAL,AUXILIAR,OCULTO
CAMPO=PESOREAL,NOUPDATE,VIRTUAL,AUXILIAR,OCULTO
CAMPO=OK,AUXILIAR,NOUPDATE,VIRTUAL,OCULTO
CAMPO=TARA,NOUPDATE,AUXILIAR,VIRTUAL,OCULTO
CAMPO=SEQLINEA,OCULTO,NOUPDATE,AUXILIAR,"@L@@@"
CAMPO=DATOSEXTRA,OCULTO,NOUPDATE,AUXILIAR,VIRTUAL
CAMPO=DATOSEXTRA2,OCULTO,NOUPDATE,AUXILIAR,VIRTUAL
CAMPO=TARAPARCIAL,AUXILIAR,NOUPDATE,VIRTUAL,OCULTO
CAMPO=ULTIMOPESO,AUXILIAR,NOUPDATE,VIRTUAL,OCULTO
CAMPO=MARGENPESO,AUXILIAR,NOUPDATE,VIRTUAL,OCULTO
CAMPO=ULTIMACANTIDAD,NOUPDATE,AUXILIAR,VIRTUAL,OCULTO
CAMPO=ULTIMOART,AUXILIAR,NOUPDATE,VIRTUAL,OCULTO
...
CURSOR=CSELCANTIDAD SELECT ART.DESART,SUM(CANTSERVIDA) CANTSERVIDA
                      FROM VDBULTOLIN BUL,VDARTIC ART
                      WHERE BUL.CODBULTO=:CODBULTOAUX AND BUL.CODART=:CODART AND ART.CODART=BUL.CODART
                      GROUP BY ART.DESART
                      HAVING SUM(CANTSERVIDA)>0;   
