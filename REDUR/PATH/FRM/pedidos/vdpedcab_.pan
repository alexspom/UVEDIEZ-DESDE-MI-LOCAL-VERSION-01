...
CAMPO=FECENTCONCERTADA,TOOLTIP("Fecha de entrada concertada") 
CAMPO=FECENTLIMITE,TOOLTIP("Fecha entrada límite")
...
CURSOR=CIMPREBULTOS BEGIN
                      FOR I IN (SELECT CODBULTO FROM VDBULTOCAB WHERE CODPED=:CODPED AND CODDIV=:CODDIV AND ANOPED=:ANOPED AND SEQPED=:SEQPED ORDER BY NBULTO) LOOP
						  ...
						  RDRPKG.IMPRIMEAMZNCCBULTO(I.CODBULTO);
...
CURSOR=CIMPRIMEAMAETI DECLARE
                    RET NUMBER;
                  BEGIN
					RDRPKG.IMPRIMEAMASN(:CODDIV,:CODPED,:ANOPED,:SEQPED);
                    COMMIT;
                  END;@
