...
BEGINBLOQUE=VDINCLUYEPEDSERIE.PAN
 ...
 WHERE=( NVL(CODSERIEPREP,0) =0 OR CODSERIEPREP=:CODSERIEPREP) AND NVL(CODSERIEEXP,0)=0 AND
      ...
      (PEDIDOCLIENTE LIKE :PEDIDOCLIENTEAUX OR :PEDIDOCLIENTEAUX IS NULL) AND 
	  (CODCLIENTE IN (SELECT CODCLIENTE FROM VDCLIENTES WHERE CODRUTA LIKE :CODRUTAAUX) OR :CODRUTAAUX IS NULL);
