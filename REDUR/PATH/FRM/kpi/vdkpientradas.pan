KPI DE ENTRADAS
FECHA      ORDENES   RECEPCIONES PALETS UNIDADES  PALMULTI CONTE40 CONTE20 PALCONTE
¿D-MM-Y.YY @L,@@@@   @L,@@@@@@   @L,@@@ @L,@@@@@@ @L,@@@   @L,@@   @L,@@@  @L,@@@
|

SOLOQUERY
SELECT=SELECT FECFINMOV,COUNT(DISTINCT CODORDREC) ORDENES,COUNT(DISTINCT CODRECEP) RECEPCIONES,SUM( CODMAT) PALETS,SUM(CANTIDAD) UNIDADES,
        SUM(NUMPALMULTI) NUMPALMULTI,SUM(CONTE40) CONTE40,SUM(CONTE20) CONTE20,SUM(CODMATCONTE) PALETSCONTENEDOR
 FROM (SELECT DAT.FECFINMOV,DAT.CODORDREC,RCC.CODRECEP,CODMAT,CODMATCONTE,CANTIDAD,RCC.NUMPALMULTI,CASE RCC.TIPOCONTENEDOR WHEN '40' THEN RCC.NUMCONTE ELSE 0 END CONTE40,
             CASE RCC.TIPOCONTENEDOR WHEN '20' THEN RCC.NUMCONTE ELSE 0 END CONTE20
    FROM (
      SELECT MOV.FECFINMOV,RCC.CODORDREC,RCC.CODRECEP,
       ...
       GROUP BY RCC.CODRECEP,RCC.CODORDREC,MOV.FECFINMOV
...
CAMPO=CONTE40,TITULO("Conte40"),WLONX=25
CAMPO=CONTE20,TITULO("Conte20"),WLONX=25
CAMPO=PALETSCONTENEDOR,TITULO("Pal.conte."),WLONX=30 