...
POSTQUERY=FIF("CSELNOTNULL",FPOSICIONABLOQUE("0"),FEJECUTA(  "CORDEN","\nERROR EN PL \nVDRECEP.VALRECEP",
																		   "CVERERROR","\nERROR,\n:MSGERROR",	                 
...
CURSOR=CSELTIPOORDEN SELECT TIPORDREC, CODDIV FROM VDORDRECCAB WHERE CODORDREC=:CODORDREC
                       UNION
                     SELECT :TIPORDREC,:CODDIV FROM DUAL WHERE :TIPORDREC='SO';

CURSOR=CSELNOTNULL SELECT :CODPROTO FROM DUAL WHERE :CODPROTO IS NULL AND :CODORDREC IS NULL AND :TIPORDREC IS NULL;
