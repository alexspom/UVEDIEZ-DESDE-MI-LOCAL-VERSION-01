...
CAMPO=CANTIDAD,TITULO("Cantidad"),WLONX=30
...
CURSOR=CBORRALINEA SELECT :SEQDEVOL FROM DUAL
                    WHERE :SEQDEVOL=0 AND :CODPRECINTO IN (SELECT CODPRECINTO FROM VDPRECINTO WHERE STATUS=VDVST.FPRCABIERTO);

CURSOR=CDELDVL DELETE VDDEVOLLIN WHERE ROWID=:MIROWID;

TECLA=F9,FEJECUTA("CBORRALINEA","NO SE PUEDE BORRAR LA LINEA\n\n\nEL PRECINTO DEBE ESTAR ABIERTO\nY LA LINEA NO PUEDE TENER CABECERA DE DEVOLUCION",
                  "+CDELDVL","",
                  +FCOMMIT,"",
                  +FPULSATECLAS("F3","F2"),"")
...
BOTON=BORRAR,40,270,60,60,"Borrar línea",vertis\destruir.png,F9,"Borrar línea",INCLUDECSS="border-style:outset;background-color:transparent"
