...
#* ====================
LINEAS DE RECEPCION
RECEPCION  LíneaREc  OrdenAso     LinOrdAso  Div.  Artículo                                     Lote            Caduci   Bloq. stock Cant. recep Muest. Ultima modificación              Observaciones
...
CAMPO=CODRECEP,NOENTER,TITULO("Recepción"), TOOLTIP("Número de recepción dentro de la misma orden de recepción")
CAMPO=SECLINREC,NOENTER,TOOLTIP("Secuencia de línea de recepción dentro de la propia recepción "),TITULO("Línea Rec.")
CAMPO=CODORDREC,VIRTUAL,NOENTER,TITULO("Orden Aso."),TOOLTIP("Orden de recepción/Compra Asociada")
CAMPO=SECLINORD,NOENTER, TITULO("Lin.Ord.Aso"),TOOLTIP("Linea de la orden de recepción/Compra asociada")
CAMPO=CODDIV,NOENTER,TOOLTIP("Código de la división"),TITULO("División"),WLONX=26
CAMPO=CODART,NOENTER,TOOLTIP("Código de artículo"),TITULO("Artículo"),POSTCHANGE=FEJECUTA("CDESART","ERROR CONSULTANDO LA DESCRIPCION DEL ARTICULO")
...
CAMPO=CANTIDADR,NOENTER,TOOLTIP("Cantidad recibida en esta recepción en registro burocratico por pantalla de PC"),TITULO("Cant. recep.")
