/*******************************************************************************
* Módulo : vdtratalotebloq.C                                                   *
* Función: Procesos y algoritmos sobre la liberación                           *
*          o bloqueo de lote.                                                  *
* Autor	 : Ricardo Fernández. Revisado por FGS para el Standrad, el 18-2-2010  *
* Revisado por:                                                                *
* Fecha  : 23-09-2009                                                          *
********************************************************************************
...
VDEXPORT int vdtratalotestk(void *ptr,char *param)  {
	...
	v10log(LOGNORMAL,"vdtratalotestk: Tratando liberación por interfase de registro %ld de matricula %s, articulo %s, lote %s, cantidad %lf, bloqueosini %s, bloqueosfin %s\n",
	...
	if (ret) {
		v10log(LOGERROR,"vdtratalotestk: No existe artículo %s lote %s\n",lblo->codart,lblo->codlot);
		return(ret);	
	}
	...
	if (*chkcantidad=='S') { // chequear cantidad del interfase y en stock
		...
		if (ret) {
			v10log(LOGERROR,"%s","vdtratalotestk: Error ejecutando PLSQL STKAUXbuscastocklote\n");
			return(ret);
		}
		if (stkaux.cantidad!=lblo->cantidad) {
			v10log(LOGERROR,"vdtratalotestk: Cantidad en stock %lf del articulo %s,lote %s y patrón bloqueo %s no coincide con la interfasada %lf\n",
				 stkaux.cantidad,lblo->codart,lblo->codlot,bloqold,lblo->cantidad);
			return(-1);
		}	
	}
	
	// realiza el cambio de calificación de lote/stock
	ret=vdnuevobloqlote(lblo->codart, lblo->codlot, bloqnew ,solostock,msjerror);
	if (ret) {
		v10log(LOGERROR,"vdtratalotestk: Error procesando registro %ld, artículo %s lote %s\n",lblo->seqbloq,lblo->codart,lblo->codlot);
		return (ret);
	}
	else { 
		...
		if (ret) {
			v10log(LOGNORMAL,"vdtratalotestk:Error actualizando registro %ld, artículo %s lote %s a estado %ld\n",lblo->seqbloq,lblo->codart,lblo->codlot,stfinal.status);
			return(0);
		}		
		v10log(LOGNORMAL,"vdtratalotestk: Procesado correctamente registro %ld, artículo %s lote %s\n",lblo->seqbloq,lblo->codart,lblo->codlot);
	}
	return(ret);
}
...
VDEXPORT int vdtratabloqconte(void *ptr,char *param)  {
	...
	if (ret) {
		v10log(LOGERROR,"vdtratabloqconte: Error procesando registro %ld, artículo %s lote %s matrícula %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codmat);
		return (ret);
	}
	else { 
		lblo->status=stfinal.status;	
		ret=LBLOactualizatratado(lblo,NOVERIFICA);
		if (ret) {
			v10log(LOGNORMAL,"vdtratabloqconte:Error actualizando registro %ld, artículo %s lote %s matrícula %s a estado %ld\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codmat,stfinal.status);
			return(0);
		}		
		v10log(LOGNORMAL,"vdtratabloqconte: Procesado correctamente registro %ld, artículo %s lote %s matrícula %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codmat);
	}
	return(ret);
}
...
VDEXPORT int vdtratabloqrecep(void *ptr,char *param)  {
	...
	v10log(LOGNORMAL,"vdtratabloqrecep: Tratando liberacion por interfase de registro %ld de recepción %s, articulo %s, lote %s, cantidad %lf, bloqueosini %s, bloqueosfin %s\n",
	...
	if (ret || stkaux.ret) {
		v10log(LOGERROR,"vdtratabloqrecep: Error procesando registro %ld, artículo %s lote %s recepción %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codrecep);
		return (-1);
	}
	else { 
		lblo->status=stfinal.status;	
		ret=LBLOactualizatratado(lblo,NOVERIFICA);
		if (ret) {
			v10log(LOGNORMAL,"vdtratabloqrecep:Error actualizando registro %ld, artículo %s lote %s recepción %s a estado %ld\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codrecep,stfinal.status);
			return(0);
		}		
		v10log(LOGNORMAL,"vdtratabloqrecep: Procesado correctamente registro %ld, artículo %s lote %s recepción %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->codrecep);
	}
	return(ret);
}
...
VDEXPORT int vdtratabloqbulto(void *ptr,char *param)  {
	...
	ret=STKAUXprocbloqueosstkbulto(lblo->bulto,lblo->codart,lblo->codlot,
								...
								"",(*chkcantidad=='S')?lblo->cantidad:0,msjerror,0,&stkaux);
	if (ret || stkaux.ret) {
		v10log(LOGERROR,"vdtratabloqbulto: Error procesando registro %ld, artículo %s lote %s bulto %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->bulto);
		return (-1);
	}
	else { 
		lblo->status=stfinal.status;	
		ret=LBLOactualizatratado(lblo,NOVERIFICA);
		if (ret) {
			v10log(LOGNORMAL,"vdtratabloqbulto:Error actualizando registro %ld, artículo %s lote %s bulto %s a estado %ld\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->bulto,stfinal.status);
			return(0);
		}		
		v10log(LOGNORMAL,"vdtratabloqbulto: Procesado correctamente registro %ld, artículo %s lote %s bulto %s\n",lblo->seqbloq,lblo->codart,lblo->codlot,lblo->bulto);
	}
	return(ret);
}
