/*
* Módulo : EOSCONSUMO.C
* Objeto: Calculo del consumo de los displays de gama EOS
*
*
...
#define MINCICLODIG 6

int  tablaascii[256]={
    /*Segmentos uno a uno que son 17 ints*/
    0XFFFF,A1,A2,A3,A4,A5,A6,A7,A8,A9,AA,AB,AC,AD,AE,AF,AP,
...
long consumoluces[MAXLUCESDISPLAY]={500,1000,1000,56000,56000,26000,1000};
long consumoinfrarojo=0;
int  init=0;
...
static void initconsumo(void)
{
    ...
    for (i=0;i<256;i++) {
       ...
       // escribe_log(LOGNORMAL,"Consumo de %d es %d\n",i,consuascii[i]);
    }
}


static long consumolucesdisplay(eosdisps *di)
{
    int i;
    long consumo=0;
  ...
  //      escribe_log(LOGERROR,"Consumo de %d es %d %d\n",i,consuascii[di->valor[i]],di->valor[i]);
    }
...
long consumolinea(eoslineas *linea)
{
    ...
    escribe_log(LOGDEBUG,"entro en consumo luces\n");
    if (init==0) initconsumo();
    for (i=0;i<linea->numdisp;i++) {
        ...
        escribe_log(LOGDEBUG,"Display %d tiene comsumo %ld %ld\n",i,consumodisp,consumoluces);
    }
    if (consumoluces==0) consumoluces=1;
    for (i=0;i<linea->lonida;i++) {
        consumo232+=cerosascii[linea->tramaida[i]];
    }
    escribe_log(LOGDEBUG,"La trama global tiene consumo de %ld\n",consumo232);
    ...
    if (linea->consumo>linea->limitacionlinea) {
        ...
        if (linea->ciclotrabajo[LUZDIGITOS]<MINCICLODIG) linea->ciclotrabajo[LUZDIGITOS]=MINCICLODIG;
    } else {
        int i;
        for (i=0;i<MAXLUCESDISPLAY;i++) linea->ciclotrabajo[i]=10;
    }
    escribe_log(LOGDEBUG,"Calculado consumo\n");
