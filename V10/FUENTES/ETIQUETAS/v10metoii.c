/*******************************************************************************
* Módulo : v10METOII.c                                                           *
* Función: Driver METOII para V10 de las etiquetadoras de ROCHE                  *
*                                                                              *
* Autor  : Varios (RIK)                                                                 *
* Revisado por:                                                                *
* Fecha  : 13-09-2011                                                          *
...
static void imprimecadenameto2(idrivers *id,char *cadena,char *fin,char *ptrsal)
{
  int  angulo;
  int font,fx,fy;
  char *ptr;
...
static void imprimebarrasmeto2(idrivers *id,char *cadena,char *fin,char **ptrsal)
{
  int  angulo;
  int  ancho;
//(1) Inicio
...
static void imprimelineameto2(idrivers *id,char **ptrsal,int lx,int ly,int ancho,int tipo)
{
...
//#pragma warn -par
static void tiraetiqmeto2(idrivers *id,char **ptrsal)
{
...
static void initetiqmeto2(idrivers *id,char **ptrsal)
{
  sprintf(*ptrsal,"<[Cc]>");
  *ptrsal+=strlen(*ptrsal);
}
...
static int statusmeto2(idrivers *id,int PUERTO,tcpipetiq *tcpip) {
  int aux;
  // return(STTERMICAOK);
  switch (st) {
...
{
  id->imprimecadena=imprimecadenameto2;
  id->imprimebarras=imprimebarrasmeto2;
  id->imprimelinea=imprimelineameto2;
  id->cambiamaxetiq=NULL;
  id->tiraetiq=tiraetiqmeto2;
  id->initetiq=initetiqmeto2;
  id->statusimpre=statusmeto2;
  id->repiteetiq=0;
}
