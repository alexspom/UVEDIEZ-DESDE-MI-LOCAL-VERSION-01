...
char *vgeneraetiqueta(idatos **lplpidatos,v10cursors *cur,char *params)
{
    ...
    if (!cur) return(NULL);
    for (mipiece=1;mipiece<=numpieces(params,";");mipiece++) {
        char nombre[MAXPARAM],cdato[MAXCADENA],asigactual[MAXCADENA]="";
        piece(params,asigactual,";",mipiece);
        piece(asigactual,nombre,"=",1);
        piece(asigactual,cdato,"=",2);
        if (*(cdato)=='\'') {
            char *ptr;
            sprintf(cdato,"%s",cdato+1);
            ptr=cdato+strlen(cdato)-1;
            if (*ptr=='\'')  *ptr=0;
        }
        lon=(int)strlen(cdato)+1;
