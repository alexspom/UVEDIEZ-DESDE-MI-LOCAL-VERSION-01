...
V10SSLIMPORT void v10initssl(int ssl, char *_keyfile, char *_password, char *_dhfile, char *_ca_list ) {

	sslactivo=ssl;
	strcopia(keyfile,_keyfile,strlen(_keyfile));
	strcopia(password,_password,strlen(_password));
	strcopia(dhfile,_dhfile,strlen(_dhfile));
	strcopia(ca_list,_ca_list,strlen(_ca_list));
}
...
static SSL_CTX *initialize_ctx(char *keyfile,char *password)  {
	...
	if(!(SSL_CTX_use_certificate_chain_file(ctx,keyfile))) v10log(LOGNORMAL,"Can't read certificate file %s\n",keyfile);

	pass=password;	
	SSL_CTX_set_default_passwd_cb(ctx,password_cb);
	if(!(SSL_CTX_use_PrivateKey_file(ctx,keyfile,SSL_FILETYPE_PEM))) v10log(LOGNORMAL,"Can't read key file %s con password %s\n",keyfile,password);

    // Load the CAs we trust
	if(!(SSL_CTX_load_verify_locations(ctx,ca_list,0))) v10log(LOGNORMAL,"Can't read CA list %s\n",ca_list);
...
static void load_dh_params(SSL_CTX *ctx,char *file)
{
	...
	if ((bio=BIO_new_file(file,"r")) == NULL) v10log(LOGNORMAL,"Couldn't open DH file");

	ret=PEM_read_bio_DHparams(bio,NULL,NULL,NULL);
	BIO_free(bio);
	if(SSL_CTX_set_tmp_dh(ctx,ret)<0) v10log(LOGNORMAL,"Couldn't set DH parameters");
}
...
static void generate_eph_rsa_key(SSL_CTX *ctx)
{
	...
	if (!SSL_CTX_set_tmp_rsa(ctx,rsa)) v10log(LOGNORMAL,"Couldn't set RSA key");
	RSA_free(rsa);
}
#endif
...
static SOCKET damesocserver(char *servicio,char *protocolo,int *miport)
{
  ...
  if (err) {
		v10log(LOGNORMAL,"Haciendo bind error %d\n",err);
		closesocket(soc);
		return(INVALID_SOCKET);
  }
  err=listen(soc,50);
...
static void erroresSSL(v10sockssls *p,int ret) {
	switch (ret) {
		case SSL_ERROR_ZERO_RETURN:
			v10log(LOGNORMAL,	"SSL_ERROR_ZERO_RETURN");
			break;
		case SSL_ERROR_WANT_READ:
			v10log(LOGNORMAL,	"SSL_ERROR_WANT_READ");
			break;
		case SSL_ERROR_WANT_CONNECT:
			v10log(LOGNORMAL,	"SSL_ERROR_WANT_CONNECT");
			break;
		case SSL_ERROR_WANT_ACCEPT:
			v10log(LOGNORMAL,	"SSL_ERROR_WANT_ACCEPT");
			break;
		case SSL_ERROR_WANT_X509_LOOKUP:
			v10log(LOGNORMAL,	"SSL_ERROR_WANT_X509_LOOKUP");
			break;
		case SSL_ERROR_SYSCALL:
			v10log(LOGNORMAL,	"SSL_ERROR_SYSCALL");
			rompepipe(p);
			break;
		case SSL_ERROR_SSL:
			v10log(LOGNORMAL,"SSL_ERROR_SSL");
			rompepipe(p);
			break;					
		}
}
...
static void tratav10sock(v10sockssls *p)
{
  ...
  while (p->v10sock->status!=STSOCKROTA || p->v10sock->reconecta) {
		if (p->v10sock->status==STSOCKREADY) {
			...
			num=select(0,&reading,NULL,NULL,&timeout);
			if (num==0) continue;
...
#ifdef V10_SLL
  if (sslactivo) {
		ret->ctx=initialize_ctx(keyfile,password);
...
static void trataopen(servidorsockssls *nuevo)
{
  ...
  while (nuevo->servidorsock->status!=STSOCKSERROMPE) {
		...
		l->valor->v10sock->threadtrata=CreateThread(NULL,10000,(LPTHREAD_START_ROUTINE)tratav10sock,(void *)l->valor,0,NULL);
		LeaveCriticalSection(&nuevo->servidorsock->critopen);
                             ...
                             trigsocks fnbreak,trigsocks fnclose)
{
  ...
  if (nuevo->servidorsock->soc!=INVALID_SOCKET) {
    ...
    nuevo->servidorsock->threadopen=CreateThread(NULL,10000,(LPTHREAD_START_ROUTINE)trataopen,(void *)nuevo,0,NULL);
  } else {
...
char *v10socksslgets(char *donde,int n,v10sockssls *p)
{
	...
	if (aux-p->v10sock->bufrec<n) n=aux-p->v10sock->bufrec;
