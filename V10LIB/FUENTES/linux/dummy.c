...
HMODULE LoadLibrary(char *nombre)
{
    ...
    if (ret==NULL) {
        char parcial[MAXPATH];
        char *ext;
        strcpy(parcial,nombre);
        ext=strchr(parcial,'.');
        if (ext) *ext=0;
        sprintf(destino,"lib%slinux.so",parcial);
        strmin(destino);
        ret=dlopen(destino,RTLD_LAZY);
    }
...
int CreateProcess(char *app, char *cmd, SECURITY_ATTRIBUTES *procatr, SECURITY_ATTRIBUTES *threatr, int hereda, int flags, void *entorno, char *directorio, STARTUPINFO *sti, PROCESS_INFORMATION *pi)
{
	...
	pi->dwProcessId =ret= ejecutacomando(proceso, flags, directorio);
...
{
	return(kill((size_t)pid, SIGKILL));
}

int pidetermina(HANDLE pid, int *status)
{
	return(kill((size_t)pid, SIGTERM));
}
                  
int _chsize(int fd,int lon)
{
    return(ftruncate(fd,lon));
}
...
{
    char var[MAXCADENA],valor[MAXCADENA];
    piece(variable,var,"=",1);
    piece(variable,valor,"=",2);
    return(setenv(var,valor,1));
}