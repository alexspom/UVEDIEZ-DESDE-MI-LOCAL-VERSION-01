...
int trataajax(v10dialogs *v,v10httpeventos *ev)
{
    ...
    if (strcmp(ajax->tipo,"F")==0) {
        ...
        if (!func) sprintf(respuesta,"</BR></BR> <b><font size='2' face='Verdana' color='white'>Error: No se encuentra la función %s en el módulo %s</font></b>",ajax->nombre,ajax->modulo);
...
{	
    ...
    *respuesta = 0;
    // si eslistener y tiene proxy, envia see_other, al servidor y puerto, con esta conexión
    if (eslistener) {
        strcopia(accesoAJAX,peticion+5,(int)(strstr(peticion,"HTTP/1.") - (peticion +5)));		
        v10log(LOGNORMAL,"Reenvío a SERVERAJAX con accesoAJAX=%s\0",accesoAJAX);
        return(v10httprespuesta(SEE_OTHER_AJAX,v10server,NULL,NULL,accesoAJAX,0));		
    } else { // si no, envia la respuesta, tras generar lo que corresponda, Query o Función
        if (b) {
            b->ajaxactivo = 1;
