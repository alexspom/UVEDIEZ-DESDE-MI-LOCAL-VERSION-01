...
static int ntriger;
char v10wwwuser[32];
char rutaxls[MAXPATH];
void (*funcxls)(char *)=NULL;
...
ftrigers *declaratriger(char *nombre,int (*fun)(bloques *b,ftrigers *ft),char *msg)
{
  ...
  ret->fun=fun;
  if (msg) {
     if (ret->param) free(ret->param);
     if (ret->tipoparam) free(ret->tipoparam);
     ret->param=calloc(1,sizeof(void *));
     ret->tipoparam=calloc(1,sizeof(int));
     ret->nparam=1;
     ret->param[0]=strdup(msg);
     ret->tipoparam[0]=PARAMCADENA;
     ret->msg=*ret->param;
   } else {
     ret->nparam=0;
     ret->param=NULL;
