			...
			switch (keyCode) {
				...
				case 37: // Left
					if (obj.isCollapsible()) {
 					...
 					if (level > 2 || level == 2 && $root.hasClass('sm-simple')) {
						obj.activatedItems[level - 2].focusSM();
					// move to previous non-disabled parent item (make sure we cycle so it might be the last item)
					} else if (!$root.hasClass('sm-simple')) {
				...
				case 38: // Up
					if (obj.isCollapsible()) {
						...
						if (level == 1 && !$root.hasClass('sm-simple') && obj.visibleSubMenus[level] && obj.opts.bottomToTopSubMenus) {
							getLastItemLink(obj.visibleSubMenus[level]).focusSM();
						} else if (level > 1 || $root.hasClass('sm-simple')) {
							getPreviousItemLink($li).focusSM();
						}
					}
					break;
				case 39: // Right
					...
					if ((level == 1 || !obj.visibleSubMenus[level]) && !$root.hasClass('sm-simple')) {
